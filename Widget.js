...
function (
  ...
  return declare([BaseWidget], {
    ...
    startup: function () {
      ...
      var traffic = new VectorTileLayer('widgets/Mapillary/mapillary-traffic.json')

      this.map.addLayer(layers)
      this.map.addLayer(traffic)

      var that = this

      // TODO: if default...
      console.log('defaultCoverage = ', this.config.defaultCoverage)
      console.log('trafficCoverage = ', this.config.trafficCoverage)

      dom.byId('mapillarysequences').checked = this.config.defaultCoverage
      layers.setVisibility(this.config.defaultCoverage)

      dom.byId('mapillarytraffic').checked = this.config.trafficCoverage
      traffic.setVisibility(this.config.trafficCoverage)
      ...
      on(dom.byId('mapillarytraffic'), 'change', function () {
        var mapillarytraffic = dom.byId('mapillarytraffic')
        traffic.setVisibility(mapillarytraffic.checked)
      })
    },
